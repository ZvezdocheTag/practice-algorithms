 ...
 * @return {number[]}
 */
export const processQueries = function(queries: number[], m: number) {
 ...
 * @return {number}
 */
export const maxIncreaseKeepingSkyline = function(grid: number[][]) {
  let maxRow = [];
  let maxCol = [];
  const newGrid: number[][] = Array.from({ length: grid.length }, () => []);
